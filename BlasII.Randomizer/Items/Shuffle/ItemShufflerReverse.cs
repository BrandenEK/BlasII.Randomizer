...
{
    internal class ItemShufflerReverse : IShuffler
    {
        public bool Shuffle(int seed, RandomizerSettings settings, Dictionary<string, string> output)
        {
            ...
            LocationPool progressionLocations = new(seed), junkLocations = new(seed);
            CreateLocationPool(progressionLocations, junkLocations, settings);

            // Create pools of all items to randomize
            ItemPool progressionItems = new(seed), junkItems = new(seed);
            ...
            FillProgressionItems(progressionLocations, progressionItems, output, inventory, seed);
        ...
        /// </summary>
        private void FillProgressionItems(LocationPool locations, ItemPool items, Dictionary<string, string> output, Blas2Inventory inventory, int seed)
        {
            ...
            var reachableLocations = new LocationPool(locations, seed);
